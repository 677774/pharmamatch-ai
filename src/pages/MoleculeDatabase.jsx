  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto w-full animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="md:col-span-2 bg-primary-container p-4 rounded-xl flex items-center justify-between text-on-primary shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">324 Matches Found</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Card 1 */}
        <div className="bg-surface-container-lowest border border-outline-variant/50 rounded-xl overflow-hidden hover:border-primary-fixed-dim hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 group">
          <div className="h-48 bg-surface-container-low/50 flex items-center justify-center p-4 border-b border-outline-variant/30 relative">
            <img 
              alt="Metformin 2D Structure" 
              className="max-h-full opacity-80 group-hover:opacity-100 transition-opacity mix-blend-multiply" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuBk1HMF4ljbQ4SPRrE_buIMlyKCxhjn9Nd8RM3bxZ7K2dgpoN1ubH1zmwOZDJo7J873Af25Obl1f2JKNvYFP9ckwG7K2xExNG5EFGcxaMEj_N3vtqHDRAV6W-Qxaw2SiUCnFfHnSvJZfJ6feRIUpi1ZW-UqgHUNHS188GgZbgFLO4m-tCUGfTpianI09HLTmXx7f8bxT39GGrYXtrx6NMmUwui0ejcuL4nbWwOUsB7ISLdvajMmEb4_dsx4YuS-vWKwImiLJzoA64hJ"
            />
            <div className="absolute top-3 right-3 bg-white px-2 py-1 rounded text-xs font-semibold text-teal-800 border border-teal-100 flex items-center gap-1 shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-teal-500"></span> FDA Approved
            </div>
          </div>
          <div className="p-5">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-headline font-bold text-lg text-primary">Metformin HCL</h3>
                <p className="text-sm text-on-surface-variant font-mono mt-1">C4H11N5</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-6">
              <div>
                ...
                <p className="text-sm font-medium text-on-surface">129.16 g/mol</p>
              </div>
              <div>
                <p className="text-xs text-outline mb-0.5">LogP</p>
                <p className="text-sm font-medium text-on-surface">-2.6</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-outline mb-0.5">SMILES</p>
                <p className="text-xs font-mono bg-surface-container-low p-2 rounded text-on-surface break-all border border-outline-variant/30">CN(C)C(=N)NC(=N)N</p>
              </div>
            </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/50 rounded-xl overflow-hidden hover:border-primary-fixed-dim hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 group">
          ...
          <div className="p-5">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-headline font-bold text-lg text-primary">Paracetamol</h3>
                <p className="text-sm text-on-surface-variant font-mono mt-1">C8H9NO2</p>
              </div>
              <button className="text-outline hover:text-primary"><span className="material-symbols-outlined">bookmark_border</span></button>
            </div>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-6">
              <div>
                <p className="text-xs text-outline mb-0.5">Mol Weight</p>
                <p className="text-sm font-medium text-on-surface">151.16 g/mol</p>
              </div>
              <div>
                <p className="text-xs text-outline mb-0.5">LogP</p>
                <p className="text-sm font-medium text-on-surface">0.46</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-outline mb-0.5">SMILES</p>
                <p className="text-xs font-mono bg-surface-container-low p-2 rounded text-on-surface break-all border border-outline-variant/30">CC(=O)NC1=CC=C(O)C=C1</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button className="flex-1 bg-primary hover:bg-[#005b6f] text-white py-2 rounded text-sm font-medium transition-colors duration-150 flex items-center justify-center gap-2">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/50 rounded-xl overflow-hidden hover:border-primary-fixed-dim hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 group">
          <div className="h-48 bg-surface-container-low/50 flex items-center justify-center p-4 border-b border-outline-variant/30 relative">
            <img 
              alt="Ibuprofen 2D Structure" 
              className="max-h-full opacity-80 group-hover:opacity-100 transition-opacity mix-blend-multiply" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuBblfsA98h76xO7Kg2Hy1BwkVNavrOagLPKJVvLLzpq-Z3cafgIY7oetzDP6-9EfEkVwETvTG_VvsmuTKM5RyXyWFk0J9ndGrCSgGZg9KcVZyZEYv1x4XF8yx8cd8cDMo04NUjEar3b5nO5R4TABR0-u_06CW66HrKbaiokz43Q8EKGnNCf3VcN1l_2RshpgqtecLxMQOpI7ebM9ceMAmgAPuyiGZ5YIxLJaczuXLXk9JE_eSwG7RtfavjAWLP6lonvZ8L9MruNdgEt"
            />
            <div className="absolute top-3 right-3 bg-white px-2 py-1 rounded text-xs font-semibold text-amber-700 border border-amber-200 flex items-center gap-1 shadow-sm">
          ...
          <div className="p-5">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-headline font-bold text-lg text-primary">Ibuprofen</h3>
                <p className="text-sm text-on-surface-variant font-mono mt-1">C13H18O2</p>
              </div>
              <button className="text-outline hover:text-primary"><span className="material-symbols-outlined">bookmark_border</span></button>
            </div>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-6">
              <div>
                <p className="text-xs text-outline mb-0.5">Mol Weight</p>
                <p className="text-sm font-medium text-on-surface">206.29 g/mol</p>
              </div>
              <div>
                <p className="text-xs text-outline mb-0.5">LogP</p>
                <p className="text-sm font-medium text-on-surface">3.97</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-outline mb-0.5">SMILES</p>
                <p className="text-xs font-mono bg-surface-container-low p-2 rounded text-on-surface break-all border border-outline-variant/30">CC(C)CC1=CC=C(C=C1)C(C)C(=O)O</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button className="flex-1 bg-primary hover:bg-[#005b6f] text-white py-2 rounded text-sm font-medium transition-colors duration-150 flex items-center justify-center gap-2">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mt-8 pt-4 border-t border-outline-variant/30 gap-4">
        <p className="text-sm text-outline">Showing 1-3 of 324 results</p>
